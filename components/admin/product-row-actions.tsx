"use client";

import { Pencil, Trash2 } from "lucide-react";
import Link from "next/link";
import { useRouter } from "next/navigation";
import { useState } from "react";

import { Button } from "@/components/ui/button";

/**
 * Per-row edit and delete.
 *
 * Delete lived only inside the edit form, below the fold — so the list, which is
 * where you actually decide what to remove, offered no way to do it. The server
 * still enforces the quote-history guard; this only surfaces the action.
 */
export function ProductRowActions({
  id,
  sku,
  name,
}: {
  id: string;
  sku: string;
  name: string;
}) {
  const router = useRouter();
  const [pending, setPending] = useState(false);
  const [error, setError] = useState<string | null>(null);

  async function remove() {
    if (
      !window.confirm(
        `Delete "${name}" (${sku})?\n\nThis cannot be undone. Photos stay in the media library.`,
      )
    ) {
      return;
    }

    setPending(true);
    setError(null);
    try {
      const response = await fetch(`/api/admin/products/${id}`, { method: "DELETE" });
      if (response.ok) {
        router.refresh();
        return;
      }

      const body = (await response.json().catch(() => ({}))) as { error?: string; quotes?: number };
      if (body.error === "referenced_by_quotes") {
        // Actionable rather than just refused: the alternative is one click away.
        const count = body.quotes ?? 0;
        const subject =
          count === 1 ? "1 quote request references" : `${count} quote requests reference`;
        setError(
          `${subject} this product, so the history would break. Open it and untick "Active" to hide it from the site instead.`,
        );
      } else if (body.error === "forbidden" || body.error === "unauthenticated") {
        setError("Your session no longer has admin access. Sign in again.");
      } else {
        setError("Could not delete that product.");
      }
    } catch {
      setError("Could not delete that product.");
    } finally {
      setPending(false);
    }
  }

  return (
    <div className="flex flex-col items-end gap-1.5">
      <div className="flex justify-end gap-1.5">
        <Link
          href={`/admin/products/${id}`}
          className="hover:bg-secondary focus-visible:ring-ring inline-flex h-8 items-center gap-1.5 rounded-lg px-2.5 text-sm font-medium transition-colors focus-visible:ring-2 focus-visible:outline-none"
        >
          <Pencil aria-hidden className="size-3.5" />
          Edit
        </Link>
        <Button
          size="sm"
          variant="ghost"
          disabled={pending}
          onClick={remove}
          className="text-destructive hover:bg-destructive/10"
        >
          <Trash2 aria-hidden className="size-3.5" />
          {pending ? "Deleting…" : "Delete"}
        </Button>
      </div>

      {error ? (
        <p role="alert" className="text-destructive max-w-64 text-right text-xs">
          {error}
        </p>
      ) : null}
    </div>
  );
}
